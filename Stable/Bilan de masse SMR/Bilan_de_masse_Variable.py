...
from Reaction_VaporeformageVariable import VaporeformageTKVariable
...
def Bilan_de_masse_TK_Variable():
    ...
    surf = ax.plot_surface(X, Y, CH_Tab.T, cmap='inferno', edgecolor='none')
    ...
    surf = ax.plot_surface(X, Y, H_Tab.T, cmap='inferno', edgecolor='none')
    fig.colorbar(surf, shrink=0.5, aspect=10)
    ax.set_xlabel('Temperature [K]')
    ax.set_ylabel('Ratio H20/CH4')
    ...
    fig = plt.figure()
    #fig.suptitle('Pureté du flux de H2 gazeux en sortie du réacteur en fonction de la température et du ratio H2O/CH4')
    ax = fig.gca(projection='3d')
    H_Purete_Tab = 100*H_Tab/(CH_Tab+H_Tab)
    surf = ax.plot_surface(X, Y, H_Purete_Tab.T, cmap='inferno', edgecolor='none')
    fig.colorbar(surf, shrink=0.5, aspect=10)
